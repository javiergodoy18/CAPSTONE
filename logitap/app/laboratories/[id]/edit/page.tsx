...
export default function EditLaboratoryPage({
  ...
  async function loadLaboratory(laboratoryId: string) {
    try {
      const response = await fetch(`/api/laboratories/${laboratoryId}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || "Error al actualizar laboratorio");
      }

      // Actualizar coordenadas en viajes activos
      try {
        await fetch(`/api/laboratories/${id}/update-dispatch-coordinates`, {
          method: "POST",
        });
      } catch (error) {
  ...
  return (
    <div className="page-container">
      <div className="page-content" style={{ maxWidth: '800px' }}>
        <h1 className="page-title">Editar Laboratorio</h1>
